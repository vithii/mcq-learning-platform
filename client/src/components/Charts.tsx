...
export const ActivityChart: React.FC<{ data: ActivityItem[] }> = ({ data }) => {
  ...
  const maxQ = Math.max(5, ...data.map(d => d.questions_answered));

  return (
    <div style={{ width: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.75rem', height: '140px', padding: '1rem 0 0.5rem 0' }}>
        {data.map((item, i) => {
          const heightPct = Math.round((item.questions_answered / maxQ) * 100);
          const dateLabel = new Date(item.date).toLocaleDateString(undefined, { weekday: 'short' });
          const isToday = new Date(item.date).toISOString().split('T')[0] === new Date().toISOString().split('T')[0];

          return (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
              <div style={{ fontSize: '0.7rem', fontWeight: 600, color: item.questions_answered > 0 ? 'var(--text-main)' : 'var(--text-dim)', marginBottom: '4px' }}>
                {item.questions_answered > 0 ? item.questions_answered : ''}
              </div>
              ...
              <div
                style={{
                  ...
                  borderRadius: '6px',
                  background: item.questions_answered > 0
                    ? isToday
                      ? 'linear-gradient(180deg, var(--primary), var(--accent))'
                      : 'linear-gradient(180deg, var(--primary-light), var(--primary))'
                    : 'rgba(255, 255, 255, 0.04)',
                  transition: 'height 0.4s ease',
                  boxShadow: item.questions_answered > 0 ? '0 2px 8px rgba(99, 102, 241, 0.25)' : 'none'
                }}
                title={`${item.date}: ${item.questions_answered} questions (${item.accuracy}% accuracy)`}
              />
